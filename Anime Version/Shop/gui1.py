...
with open("Files/Temp Files/Shop Temp.csv", 'r') as csv_open:
    ...
    for k in fr:
        ...
        rank=k[2]
        desc=k[3]
        # Split description into two lines for the UI
...
# --- MODIFIED: This function now handles both Quests and Items ---
def shop_check(name, cat, rank, val):
    """
    Checks if the player can afford the item and processes the purchase.
    """
    if coins >= int(val):
        # If the item is a quest, add it to the quest log
        if cat == "QUEST":
            thesystem.shop.quests_add(rank, val, read_status_file_data, window)
        # Otherwise, it's an item/equipment, so add it to inventory
        else:
            item_to_add = {
                "name": name,
                "category": cat,
            ...
            # We call a new function to handle adding to inventory and deducting coins
            thesystem.shop.inventory_add(item_to_add, val, read_status_file_data, window)
    else:
        # Optional: You can add a message here if the player doesn't have enough coins
...
button_1 = Button(
    ...
    borderwidth=0,
    highlightthickness=0,
    # --- MODIFIED: Pass the category ('cat') to the shop_check function ---
    command=lambda: shop_check(name, cat, rank, value),
    relief="flat"
)
button_1.place(
    x=370.0,
